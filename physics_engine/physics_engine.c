...
void handlePauseInput(char *input);
void handleResumeInput(char *input);
...
void updatePositions()
{
    for (int i = 0; i < size; i++)
    {
        ...
        if (circles[i].node.pos.x + circles[i].radius <= 0)
            ...
            circles[i].alive = 0;
        else if (circles[i].node.pos.x - circles[i].radius >= WINDOW_WIDTH)
            circles[i].alive = 0;
        else if (circles[i].node.pos.y - circles[i].radius >= WINDOW_HEIGHT)
            circles[i].alive = 0;
    }
}
